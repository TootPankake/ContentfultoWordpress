...
# exponential backoff
# categories from contentful (only category type of activity) all added as activity
# already 10+ entries linked to activity in the right tab on contentful (make sure to filter only articles)
# link to articles instead of barriers -^
# fill categories with blank descriptions for now, maybe comment out, then later when content is generated it fill them
# remove other pages for now
...
for entry in all_categories:
    ...
    if category_type == "Activity":
        create_grandparent_page(title, description, category_id)
...
print(f"Loaded {len(processed_articles)} processed articles from JSON.")
