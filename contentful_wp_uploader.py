...
site_url = 'https://staging-b84a-kyled09aa086b4c.wpcomstaging.com/'
...
def fetch_metadata_id(existing_pages):
    for page in existing_pages:
        page_id = page['id']
        # Fetch metadata for each page
        meta_url = f'{site_url}/wp-json/wp/v2/pages/{page_id}'
        ...
        if meta_response.status_code == 200:
            ...
            if metadata_id:
                existing_metadata.append({'id': page_id, 'metadata_id': metadata_id})
...
fetch_metadata_id(existing_pages)

if refreshArticles == 'Y':
    date_threshold = datetime(2024, 1, 1).isoformat()
    date_threshold_articles = datetime(2023, 1, 1).isoformat() # change to 2023, 1, 1
else: 
...
for activity in sorted(activity_types):
    for entry2 in all_activities:
        ...
        if content and addActivities == 'Y':
            ...
            body = ""
    create_parent_page(activity, body, id)
    iteration +=1
print(iteration)
